...
{
#ifdef CONFIG_BCMDHD88_FW_PATH
	bcm_strncpy_s(fw_path, sizeof(fw_path), CONFIG_BCMDHD88_FW_PATH, MOD_PARAM_PATHLEN-1);
#else /* CONFIG_BCMDHD88_FW_PATH */
	fw_path[0] = '\0';
#endif /* CONFIG_BCMDHD88_FW_PATH */
#ifdef CONFIG_BCMDHD88_NVRAM_PATH
	bcm_strncpy_s(nv_path, sizeof(nv_path), CONFIG_BCMDHD88_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
#else /* CONFIG_BCMDHD88_NVRAM_PATH */
	nv_path[0] = '\0';
#endif /* CONFIG_BCMDHD88_NVRAM_PATH */
#ifdef SOFTAP
	fw_path2[0] = '\0';
#endif
}
...
dhd_arp_offload_set(dhd_pub_t * dhd, int arp_mode)
{
	...
	retcode = dhd_iovar(dhd, 0, "arp_ol", (char *)&arp_mode, sizeof(arp_mode),
			NULL, 0, TRUE);
...
dhd_arp_offload_enable(dhd_pub_t * dhd, int arp_enable)
{
	...
	retcode = dhd_iovar(dhd, 0, "arpoe", (char *)&arp_enable,
	...
	if (arp_enable) {
		uint32 version;
		retcode = dhd_iovar(dhd, 0, "arp_version", NULL, 0, (char *)&iovbuf,
				sizeof(iovbuf), FALSE);
		if (retcode) {
...
dhd_arp_offload_add_ip(dhd_pub_t *dhd, uint32 ipaddr, int idx)
{
	...
	if (dhd->arp_version == 1)
		idx = 0;
	retcode = dhd_iovar(dhd, 0, "arp_hostip", (char *)&ipaddr, sizeof(ipaddr),
			NULL, 0, TRUE);
...
dhd_arp_get_arp_hostip_table(dhd_pub_t *dhd, void *buf, int buflen, int idx)
{
	...
	retcode = dhd_iovar(dhd, 0, "arp_hostip", NULL, 0, (char *)buf, buflen,
			FALSE);
...
dhd_ndo_enable(dhd_pub_t * dhd, int ndo_enable)
{
	...
	retcode = dhd_iovar(dhd, 0, "ndoe", (char *)&ndo_enable, sizeof(ndo_enable),
			NULL, 0, TRUE);
	if (retcode)
...
dhd_ndo_add_ip(dhd_pub_t *dhd, char* ipv6addr, int idx)
{
	...
	retcode = dhd_iovar(dhd, 0, "nd_hostip", (char *)ipv6addr, IPV6_ADDR_LEN,
			NULL, 0, TRUE);
...
dhd_ndo_remove_ip(dhd_pub_t *dhd, int idx)
{
	...
	retcode = dhd_iovar(dhd, 0, "nd_hostip_clear", NULL, 0, NULL, 0, TRUE);
